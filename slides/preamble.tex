...
\newfontfamily\mathsymbolfont{Noto Sans Math}
\usepackage[vietnamese]{babel}

% Custom commands for symbols
\newcommand{\arrowright}{{\mathsymbolfont →}}
\newcommand{\Deltasym}{{\mathsymbolfont Δ}}
\newcommand{\timessym}{{\mathsymbolfont ×}}
\newcommand{\mdot}{{\mathsymbolfont ṁ}}
...
\defbeamertemplate*{title page}{neubrutalism}[1][]{
    \begin{tikzpicture}[remember picture, overlay]
        ...
        \node[anchor=north] at ([yshift=-0.18\paperheight]current page.north) {
            \includegraphics[height=1.4cm]{../assets/bk_logo_transparent.png}
        };
        
        % Subtitle (on top)
        \node[anchor=north, text=bkblue, font=\normalsize] at ([yshift=-0.35\paperheight]current page.north) {
            \insertsubtitle
        };
        
        % Title (2 lines, below subtitle)
        \node[anchor=north, align=center, text=darkblue, font=\large\bfseries] at ([yshift=-0.44\paperheight]current page.north) {
            Điều khiển tối ưu hoá năng lượng cung cấp\\cho quá trình chưng cất hệ ethanol -- nước
        };
        
        % Author info
        \node[anchor=north, text=darkblue, font=\small, align=center] at ([yshift=-0.62\paperheight]current page.north) {
            SVTH: Nguyễn Phúc Bảo Lâm -- MSSV: 2111630\\GVHD: TS. Bùi Ngọc Pha
        };
    \end{tikzpicture}
}
