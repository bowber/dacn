...
\begin{itemize}
    ...
    \item Hysteresis (deadband) = $\pm 1^\circ$C (heater ON khi PV $< 39^\circ$C, OFF khi PV $> 41^\circ$C)
...
% PV line
\addplot[blue, thick] table[x=time_s, y=PV, col sep=comma] {../tmp/TN2_processed.csv};
\addlegendentry{PV (Nhiệt độ đo được)}

% SV line
\addplot[red, dashed, thick] table[x=time_s, y=SV, col sep=comma] {../tmp/TN2_processed.csv};
...
\begin{itemize}
    ...
    \item \textbf{Quá điều chỉnh (131-176s):} Heater OFF nhưng nhiệt độ tiếp tục tăng do quán tính nhiệt, đạt đỉnh $45.5^\circ$C (vượt SV $5.5^\circ$C)
...
\hline
1 & $\approx 6.5$ (peak-to-peak $\approx 13$) & $\approx 250$ & $\approx 0.24$/phút \\
\hline
2 & -- (chưa khảo sát) & -- & -- \\
\hline
5 & -- (chưa khảo sát) & -- & -- \\
...
\begin{itemize}
    ...
    \item Độ quá điều chỉnh: $\approx 14\%$ ($(45.5-40)/40 \times 100$)
...
\begin{itemize}
    ...
    \item Độ quá điều chỉnh $\approx 14\%$ là do heater không tắt ngay khi đạt SV mà phải đợi vượt ngưỡng $41^\circ$C
\end{itemize}

\subsubsection{Ảnh hưởng của hysteresis đến chất lượng điều khiển}

\begin{itemize}
    \item \textbf{Hysteresis nhỏ:} Giảm biên độ dao động nhưng tăng tần số đóng ngắt, gây hao mòn thiết bị
    \item \textbf{Hysteresis lớn:} Giảm tần số đóng ngắt nhưng tăng biên độ dao động, giảm độ chính xác
    \item \textbf{Giá trị tối ưu:} Cần cân bằng giữa độ chính xác và tuổi thọ thiết bị
\end{itemize}

\subsubsection{So sánh với điều khiển PID}

\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|}
\hline
\textbf{Tiêu chí} & \textbf{ON/OFF} & \textbf{PID} \\
\hline
Độ phức tạp & Thấp & Cao \\
\hline
Biên độ dao động & Lớn ($\pm 6-7^\circ$C) & Nhỏ ($< 2^\circ$C) \\
\hline
Sai lệch tĩnh & Không (nhưng dao động) & Triệt tiêu được \\
\hline
Độ mài mòn cơ cấu & Cao (đóng ngắt liên tục) & Thấp (điều khiển liên tục) \\
\hline
Ứng dụng & Yêu cầu đơn giản & Yêu cầu chính xác cao \\
\hline
\end{tabular}
\caption{So sánh điều khiển ON/OFF và PID}
\end{table}

\subsubsection{Ứng dụng trong nhận dạng hệ thống}
...
\begin{enumerate}
    ...
    \item \textbf{Dao động và quá điều chỉnh:} Với deadband $\pm 1^\circ$C, hệ thống dao động với biên độ $\approx 6.5^\circ$C và chu kỳ $\approx 250$s. Độ quá điều chỉnh $\approx 14\%$ do quán tính nhiệt.
    
    \item \textbf{Vai trò của hysteresis:} Deadband giúp giảm tần số đóng ngắt, bảo vệ thiết bị chấp hành, nhưng làm tăng biên độ dao động.
...
Hysteresis (vùng trễ) cần thiết vì:
\begin{itemize}
    \item \textbf{Tránh dao động tần số cao:} Không có hysteresis, relay sẽ đóng ngắt liên tục khi PV ở gần SP do nhiễu đo lường
