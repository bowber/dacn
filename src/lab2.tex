...
\begin{enumerate}
    ...
    \item Kết nối cảm biến nhiệt độ với analog input.
...
\textbf{Câu 1: Anh/chị hãy trình bày ưu và nhược điểm thuật toán điều khiển ON/OFF và PID.}

\textbf{Điều khiển ON/OFF:}
...
\begin{itemize}
    ...
    \item Dễ bảo trì và sửa chữa
\end{itemize}

\textit{Nhược điểm:}
\begin{itemize}
    \item Biến điều khiển dao động liên tục, không đạt được giá trị chính xác
    \item Gây mài mòn cơ cấu chấp hành do đóng ngắt liên tục
    \item Tiêu tốn năng lượng do khởi động thiết bị nhiều lần
    \item Không phù hợp với các quá trình yêu cầu điều khiển chính xác
\end{itemize}

\textbf{Điều khiển PID:}

\textit{Ưu điểm:}
\begin{itemize}
    \item Triệt tiêu sai lệch tĩnh nhờ thành phần tích phân (I)
    \item Đáp ứng nhanh nhờ thành phần tỉ lệ (P)
    \item Giảm độ quá điều chỉnh và cải thiện ổn định nhờ thành phần vi phân (D)
    \item Điều khiển mượt mà, không gây dao động như ON/OFF
    \item Có thể điều chỉnh thông số để tối ưu cho từng hệ thống cụ thể
\end{itemize}

\textit{Nhược điểm:}
\begin{itemize}
    \item Cần chỉnh định thông số ($K_P$, $T_i$, $T_d$) phù hợp
    \item Chi phí cao hơn (cần bộ điều khiển có khả năng tính toán)
    \item Phức tạp hơn trong triển khai và bảo trì
    \item Thành phần vi phân nhạy cảm với nhiễu đo lường
    \item Không phù hợp với các hệ thống phi tuyến mạnh
\end{itemize}

\textbf{So sánh tổng hợp:}
...
\hline
Độ chính xác & Thấp (dao động) & Cao \\
\hline
Sai lệch tĩnh & Có (dao động) & Triệt tiêu \\
\hline
Tuổi thọ thiết bị & Giảm (đóng ngắt nhiều) & Cao hơn \\
\hline
Ứng dụng & Hệ thống đơn giản & Hệ thống yêu cầu cao \\
\hline
\end{tabular}
