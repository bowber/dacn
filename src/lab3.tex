...
\begin{tikzpicture}[node distance=1.8cm]
    ...
    \node[block, right=1.2cm of sum, minimum width=5em] (controller) {ON-OFF};
    \node[block, right=1.5cm of controller] (process) {Quá trình};
    \node[right=1.5cm of process] (output) {$Y$};
    
    % Sensor
    \node[block, below=1cm of process] (sensor) {Cảm biến};
    ...
    \node[right=3cm of output] (char) {};
    \begin{scope}[shift={(9,0)}]
        \draw[->] (-0.5,0) -- (1.5,0) node[right] {$e$};
        \draw[->] (0,-0.8) -- (0,1.2) node[above] {$u$};
        \draw[thick] (-0.3,-0.5) -- (0,-0.5) -- (0,0.7) -- (1,0.7);
        \node[font=\tiny] at (1.2,0.7) {$u_{max}$};
        \node[font=\tiny] at (0.5,-0.5) {$u_{min}$};
    \end{scope}
\end{tikzpicture}
\caption{Sơ đồ khối và đặc tính bộ điều khiển ON-OFF}
\label{fig:onoff_control}
\end{figure}
...
\begin{tikzpicture}[node distance=1.5cm]
    ...
    % Sum
    \node[sumcircle, right=1.5cm of I] (sum) {};
    ...
    \node[font=\tiny] at ($(sum.west)+(0.2,0)$) {$+$};
