...
Hệ ethanol -- nước là hệ hai cấu tử không lý tưởng, có điểm đẳng phí ở nồng độ khoảng 95.6\% mol ethanol (89.4\% khối lượng) tại áp suất khí quyển \cite{gmehling1991, crc2020}.
...
\hline
\end{tabular}
\caption{Tính chất vật lý của ethanol và nước \cite{crc2020}}
...
% Đường hơi (dew point)
\addplot[black, thick, dashed, smooth] coordinates {
...
\subsubsection{Điều khiển công suất bằng PWM}

Điều chế độ rộng xung (PWM -- Pulse Width Modulation) là phương pháp điều khiển công suất bằng cách thay đổi tỷ lệ thời gian đóng/ngắt của tín hiệu điều khiển trong một chu kỳ cố định.
...
Trong đó:
\begin{itemize}
    \item $t_{ON}$: Thời gian tín hiệu ở mức cao (ON)
    \item $T$: Chu kỳ PWM ($T = t_{ON} + t_{OFF}$)
\end{itemize}

Công suất trung bình đầu ra tỷ lệ với duty cycle:
\begin{equation}
    P_{avg} = P_{max} \times \frac{t_{ON}}{T}
\end{equation}

\textbf{Ưu điểm của PWM so với điều khiển liên tục:}
\begin{itemize}
    \item \textbf{Hiệu suất cao:} Phần tử đóng ngắt (relay, SSR) hoạt động ở chế độ bão hòa, tổn hao công suất thấp.
    \item \textbf{Đơn giản:} Chỉ cần tín hiệu ON/OFF, không cần bộ biến đổi công suất phức tạp.
    \item \textbf{Tương thích PLC:} Dễ dàng triển khai trên PLC với ngõ ra số (digital output).
\end{itemize}

\textbf{Ứng dụng trong điều khiển reboiler:}

Reboiler điện sử dụng điện trở gia nhiệt, là tải thuần trở phù hợp với điều khiển PWM. Với chu kỳ PWM đủ lớn (vài giây đến vài chục giây), quán tính nhiệt của hệ thống sẽ lọc dao động, cho công suất nhiệt ổn định.

Ví dụ: Với reboiler 6000 W và duty cycle 50\%:
\begin{equation}
    P_{avg} = 6000 \times 0.5 = 3000 \text{ W}
\end{equation}
...
Khi điều khiển van làm mát theo áp suất, lưu lượng nước làm mát được giảm xuống mức vừa đủ:
\begin{equation}
    \eta_{savings} = \frac{F_{baseline} - F_{optimal}}{F_{baseline}} \times 100\%
\end{equation}

Trong đó:
\begin{itemize}
    \item $F_{baseline}$: Lưu lượng nước làm mát khi van mở 100\%
    \item $F_{optimal}$: Lưu lượng nước làm mát khi vận hành tối ưu
\end{itemize}

Ngoài ra, việc điều khiển PWM công suất reboiler cho phép giảm điện năng tiêu thụ:
\begin{equation}
    P_{avg} = P_{max} \times \text{duty cycle}
\end{equation}

Duty cycle tối ưu được xác định bằng thực nghiệm, đảm bảo duy trì chất lượng sản phẩm trong khi tiết kiệm điện năng.
