...
Phương pháp McCabe-Thiele (Green \& Southard, 2019) là phương pháp đồ thị để xác định số mâm lý thuyết cần thiết cho quá trình chưng cất. Phương pháp này dựa trên hai giả thiết đơn giản hóa:
\begin{itemize}
...
Với hệ ethanol--nước, nhiệt hóa hơi của ethanol (38.56 kJ/mol) và nước (40.66 kJ/mol) chênh lệch khoảng 5\%, nằm trong phạm vi chấp nhận được cho phương pháp McCabe-Thiele (thường $<$10\%) (Green \& Southard, 2019).
...
\subsubsection{Xác định tỷ số hồi lưu vận hành}

Với hệ thống thí nghiệm có 6 mâm thực tế và hiệu suất Murphree $E_{MV} = 80\%$, số mâm lý thuyết khả dụng:
\begin{equation}
    N_{\text{khả dụng}} = N_{\text{thực tế}} \times E_{MV} = 6 \times 0.8 = 4.8 \text{ mâm lý thuyết}
\end{equation}

Phương pháp McCabe-Thiele cho thấy số mâm lý thuyết cần thiết phụ thuộc vào tỷ số hồi lưu $R$: khi $R$ tăng, số mâm cần thiết giảm (và ngược lại). Với các thông số vận hành:
\begin{itemize}
    \item Nồng độ nhập liệu: $x_F = 10\%$ vol (3.32\% mol)
    \item Nồng độ sản phẩm đỉnh: $x_D = 90\%$ vol (73.56\% mol)
    \item Nồng độ sản phẩm đáy: $x_W = 1\%$ vol (0.31\% mol)
    \item Hệ số q = 1.042 (nhập liệu lạnh ở 70$^\circ$C)
\end{itemize}

Tỷ số hồi lưu tối thiểu được xác định từ giao điểm của đường q với đường cân bằng:
\begin{equation}
    R_{min} = \frac{x_D - y_{pinch}}{y_{pinch} - x_{pinch}} = 1.68
\end{equation}

Từ phân tích McCabe-Thiele với các giá trị $R$ khác nhau:

\begin{table}[H]
\centering
\begin{tabular}{|c|c|c|}
\hline
\textbf{Tỷ số hồi lưu $R$} & \textbf{Số mâm lý thuyết $N$} & \textbf{$R/R_{min}$} \\
\hline
7.7 & 5 & 4.6 \\
\hline
10.4 & 4.8 (nội suy) & 6.2 \\
\hline
35.1 & 4 & 20.9 \\
\hline
\end{tabular}
\caption{Quan hệ giữa tỷ số hồi lưu và số mâm lý thuyết}
\label{tab:reflux_stages}
\end{table}

Vì phương pháp McCabe-Thiele cho kết quả số nguyên (đếm bậc thang), giá trị $R$ cho $N = 4.8$ được xác định bằng nội suy logarit giữa hai biên:
\begin{equation}
    R_{4.8} = R_5 \times \left(\frac{R_4}{R_5}\right)^{(5-4.8)/(5-4)} = 7.7 \times \left(\frac{35.1}{7.7}\right)^{0.2} = 10.4
\end{equation}

Do đó, tỷ số hồi lưu vận hành được chọn là $R \approx 10$ (làm tròn cho thực tế), tương đương $6.2 \times R_{min}$. Giá trị này đảm bảo:
\begin{itemize}
    \item Đủ để đạt nồng độ sản phẩm 90\% vol với 6 mâm thực tế
    \item Không quá cao gây lãng phí năng lượng
    \item Có biên độ an toàn cho các nhiễu loạn vận hành
\end{itemize}
...
% Đường chéo y = x (không có legend)
\addplot[gray, dashed, thin, forget plot] coordinates {(0,0) (1,1)};

% Đường cân bằng ethanol-nước (tính từ phương trình Antoine + Van Laar)
\addplot[blue, thick, smooth] coordinates {
    (0.0000, 0.0000) (0.0503, 0.3246) (0.1005, 0.4410) (0.1508, 0.4999)
    (0.2010, 0.5360) (0.2513, 0.5619) (0.3015, 0.5830) (0.3518, 0.6020)
    (0.4020, 0.6205) (0.4523, 0.6394) (0.5025, 0.6594) (0.6030, 0.7046)
    (0.7035, 0.7588) (0.8040, 0.8248) (0.9045, 0.9056) (1.0000, 1.0000)
};
\addlegendentry{Đường cân bằng}

% Đường vận hành phần cất (R = 10.4, x_D = 0.7356)
% y = 0.9123*x + 0.0645
\addplot[red, thick] coordinates {(0.0358, 0.0972) (0.7356, 0.7356)};
\addlegendentry{Đường vận hành phần cất}

% Đường vận hành phần chưng
\addplot[orange, thick] coordinates {(0.0031, 0.0031) (0.0358, 0.0972)};
\addlegendentry{Đường vận hành phần chưng}

% Đường q (q = 1.042, x_F = 0.0332)
\addplot[green!60!black, thick, dashed] coordinates {(0.0332, 0.0332) (0.0358, 0.0972)};
\addlegendentry{Đường q}

% Các bậc thang (5 mâm lý thuyết)
\addplot[black, thin, forget plot] coordinates {
    (0.7356, 0.7356)
    (0.6629, 0.7356)
    (0.6629, 0.6693)
    (0.5259, 0.6693)
...
\addplot[only marks, mark=*, mark size=2pt, black, forget plot] coordinates {
    (0.7356, 0.7356) (0.0332, 0.0332) (0.0031, 0.0031) (0.0358, 0.0972)
};

% Labels
\node at (axis cs:0.80, 0.70) {\small $x_D$};
\node at (axis cs:0.08, 0.08) {\small $x_F$};
\node at (axis cs:0.05, 0.00) {\small $x_W$};

\end{axis}
\end{tikzpicture}
\caption{Phương pháp McCabe-Thiele với $R = 10.4$ (đồ thị vẽ 5 bậc; tương đương 4.8 mâm lý thuyết khi nội suy)}
...
Với mâm xuyên lỗ trong công nghiệp, $E_{MV}$ thường nằm trong khoảng 70--80\% (Green \& Southard, 2019). Số mâm thực tế cần thiết:
\begin{equation}
...
\hline
\end{tabular}
\caption{Tính chất vật lý của ethanol và nước (NIST, n.d.)}
