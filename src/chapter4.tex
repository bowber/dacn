...
Từ phương trình truyền nhiệt \cite{perry2019}:
\begin{equation}
...
Trong đó $U_{ref}$ là hệ số truyền nhiệt tại lưu lượng tham chiếu $F_{ref} = 7.2$ L/min (van mở 100\%). Số mũ 0.8 xuất phát từ tương quan Dittus-Boelter cho dòng chảy rối trong ống ($Nu \propto Re^{0.8}$) \cite{incropera2011}.

\textbf{Lựa chọn giá trị $U_{ref}$:} Theo \cite{engineering_toolbox_2003}, hệ số truyền nhiệt tổng cho hệ thống ngưng tụ hơi hữu cơ với nước làm mát nằm trong khoảng 300--1200 W/(m²·K). Giá trị thấp (300--500) áp dụng cho thiết bị bám bẩn, cáu cặn; giá trị cao (1000--1200) cho thiết bị sạch. Hệ thống ethanol--nước là hệ sạch (không gây cáu cặn, không ăn mòn với SS304), do đó có thể sử dụng giá trị cao. Tuy nhiên, để đảm bảo tính bảo thủ trong tính toán, chọn $U_{ref} = 800$ W/(m²·K) -- giá trị trung bình của khoảng điển hình.
...
\hline
Chế độ cơ sở (100\% van) & 9113 W & 100\% & 7.2 L/min \\
\hline
\end{tabular}
...
\textbf{Nhận xét:} Với công suất reboiler hiệu dụng 5400 W, lưu lượng nước làm mát tối ưu là khoảng 53\% (3.8 L/min), tiết kiệm 47\% so với mở van 100\%. Chế độ cơ sở (van mở 100\%) cung cấp công suất ngưng tụ 9113 W, vượt quá nhu cầu thực tế gần gấp đôi.

\subsubsection{Điều khiển công suất reboiler bằng PWM}

Reboiler hiện tại chạy 100\% công suất (6000 W), nhưng thực tế có thể không cần nhiều như vậy. Việc giảm công suất reboiler sẽ:
\begin{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Nồng độ sản phẩm đỉnh (quan trọng nhất)
        \item Nhiệt độ đỉnh tháp (tăng khi phân tách kém)
        \item Áp suất đỉnh tháp (cần ổn định)
    \end{itemize}
    
    \item \textbf{Xác định điểm giới hạn:} Duty cycle tại đó chất lượng sản phẩm bắt đầu giảm dưới 90\% vol hoặc vận hành trở nên không ổn định.
    
    \item \textbf{Thêm biên độ an toàn:} Tăng duty cycle lên 10\% so với điểm giới hạn để đảm bảo vận hành ổn định.
\end{enumerate}

\textbf{Ước tính sơ bộ:}

Dựa trên kinh nghiệm vận hành các hệ thống tương tự, duty cycle tối ưu thường nằm trong khoảng 50--70\% cho hệ thống chưng cất quy mô phòng thí nghiệm. Giá trị chính xác phụ thuộc vào:
\begin{itemize}
...
Ví dụ: Nếu duty cycle tối ưu là 60\%, công suất reboiler giảm từ 6000 W xuống 3600 W, van làm mát có thể giảm xuống khoảng 35--40\% (so với 53\% khi reboiler chạy 100\%).

% ------------------------------------------------------------
\subsection{Kết quả tính toán trạng thái xác lập}
% ------------------------------------------------------------

Dựa trên các phương trình cân bằng năng lượng và truyền nhiệt, tính toán trạng thái xác lập cho hai chế độ vận hành: cơ sở (van mở 100\%) và vận hành tối ưu với điều khiển.

\subsubsection{Chế độ cơ sở (van mở 100\%)}

Trong chế độ vận hành cơ sở, van nước làm mát được mở hoàn toàn (100\%) -- đây là cách vận hành truyền thống không có điều khiển tự động.
...
\subsubsection{Đề xuất: Điều khiển PWM reboiler (công việc tương lai)}

Ngoài điều khiển van làm mát, có thể kết hợp thêm điều khiển PWM công suất reboiler để tối ưu hóa cả điện năng. Phương pháp xác định duty cycle tối ưu được đề xuất trong mục 4.1.5. Đây là nội dung sẽ được triển khai trong Khóa luận Tốt nghiệp.
...
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Thông số} & \textbf{Cơ sở} & \textbf{Điều khiển van} & \textbf{Đơn vị} \\
\hline
...
\textbf{Kết quả chính:}
\begin{itemize}
    \item \textbf{Điều khiển van (so với cơ sở):} Giảm 47\% lưu lượng nước làm mát trong khi duy trì áp suất đỉnh tháp ổn định.
\end{itemize}
