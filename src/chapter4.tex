...
Để vận hành tối ưu với $Q_{condenser} = Q_{vaporize} = 5282$ W, lưu lượng nước cần thiết phụ thuộc vào nhiệt độ nước ra mong muốn:
\begin{equation}
    \dot{m}_{optimal} = \frac{Q_{vaporize}}{C_p \cdot (T_{out} - T_{in})}
\end{equation}

Giới hạn nhiệt độ nước ra là nhiệt độ ngưng tụ ($T_{condensation} = 78^\circ$C). Lưu lượng tối thiểu lý thuyết:
\begin{equation}
    \dot{m}_{min} = \frac{5282}{4186 \times (78 - 30)} = 0.026 \text{ kg/s} = 1.6 \text{ L/min} \quad (\text{van } 22\%)
\end{equation}

Thực tế cần biên độ an toàn để đảm bảo truyền nhiệt hiệu quả. Bảng \ref{tab:condenser_modes} trình bày các phương án với các mức biên độ khác nhau:

\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Chế độ} & \textbf{$T_{out}$} & \textbf{$\Delta T$} & \textbf{Lưu lượng} & \textbf{Valve} \\
\hline
Baseline (van 100\%) & 40.5$^\circ$C & 37.5$^\circ$C & 7.2 L/min & 100\% \\
\hline
Tối ưu (khuyến nghị) & 50$^\circ$C & 28$^\circ$C & 3.8 L/min & 53\% \\
\hline
Tối ưu (tiết kiệm hơn) & 60$^\circ$C & 18$^\circ$C & 2.5 L/min & 35\% \\
\hline
Giới hạn lý thuyết & 78$^\circ$C & 0$^\circ$C & 1.6 L/min & 22\% \\
\hline
\end{tabular}
\caption{Lưu lượng nước làm mát theo nhiệt độ nước ra (với $Q = 5282$ W)}
\label{tab:condenser_modes}
\end{table}

\textbf{Khuyến nghị:} Chọn $T_{out} = 50^\circ$C ($\Delta T = 28^\circ$C so với nhiệt độ ngưng tụ) để đảm bảo an toàn và ổn định. Lưu lượng tương ứng là 3.8 L/min (van 53\%), tiết kiệm 47\% so với baseline.
