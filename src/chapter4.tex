...
\begin{itemize}
    ...
    \item Công suất reboiler điện: 6000 W (2 $\times$ 3000 W)
\end{itemize}
...
Nhiệt cần thiết để nâng nhiệt độ nhập liệu từ 70$^\circ$C đến điểm sôi. Điểm sôi của hỗn hợp 10\% vol ethanol (3.32\% mol) tại 1 bar được tính từ phương trình Antoine kết hợp hệ số hoạt độ Van Laar [3]:
\begin{equation}
...
\subsubsection{Tính toán công suất reboiler tối ưu (PWM)}

Phân tích trên cho thấy reboiler đang chạy 100\% công suất (6000 W), nhưng thực tế có thể không cần nhiều như vậy. Câu hỏi đặt ra: \textit{Công suất reboiler tối thiểu cần thiết là bao nhiêu?}

\textbf{Xác định công suất reboiler tối ưu:}

Công suất reboiler cần thiết phụ thuộc vào năng suất sản phẩm mong muốn. Với năng suất thiết kế và nhập liệu 4.8 L/h ở 10\% vol:

\begin{itemize}
    \item Nhiệt gia nhiệt nhập liệu: $Q_{feed} = 118$ W (cố định theo lưu lượng nhập liệu)
    \item Nhiệt hóa hơi: $Q_{vaporize}$ phụ thuộc vào lượng hơi cần tạo ra
\end{itemize}

Giả sử hệ thống chỉ cần một nửa lượng hơi hiện tại để đạt năng suất mong muốn:
\begin{equation}
    Q_{vaporize,optimal} = \frac{Q_{vaporize,current}}{2} = \frac{5282}{2} = 2641 \text{ W}
\end{equation}

Công suất reboiler tối ưu:
\begin{equation}
    Q_{reboiler,optimal} = Q_{feed} + Q_{vaporize,optimal} = 118 + 2641 = 2759 \text{ W}
\end{equation}

\textbf{Tính toán duty cycle PWM:}

Với hiệu suất reboiler 90\%, công suất điện cần thiết:
\begin{equation}
    P_{electrical,optimal} = \frac{Q_{reboiler,optimal}}{0.90} = \frac{2759}{0.90} = 3066 \text{ W}
\end{equation}

Duty cycle PWM:
\begin{equation}
    \text{Duty cycle} = \frac{P_{electrical,optimal}}{P_{electrical,max}} = \frac{3066}{6000} = 51\%
\end{equation}

\textbf{Tiềm năng tiết kiệm năng lượng:}

\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Chế độ} & \textbf{Công suất điện} & \textbf{PWM} & \textbf{Tiết kiệm} \\
\hline
Baseline (100\%) & 6000 W & 100\% & -- \\
\hline
Tối ưu (PWM) & 3066 W & 51\% & 49\% \\
\hline
\end{tabular}
\caption{So sánh công suất reboiler giữa các chế độ vận hành}
\label{tab:reboiler_pwm}
\end{table}

\textbf{Lưu ý quan trọng:} Giá trị 51\% là ước tính dựa trên giả thiết giảm 50\% lượng hơi. Giá trị thực tế cần được xác định qua thực nghiệm, phụ thuộc vào:
\begin{itemize}
    \item Năng suất sản phẩm mong muốn
    \item Chất lượng sản phẩm (nồng độ ethanol)
    \item Điều kiện vận hành thực tế
\end{itemize}

\textbf{Kết hợp điều khiển van làm mát và PWM reboiler:}

Khi giảm công suất reboiler bằng PWM, lượng hơi tạo ra giảm, dẫn đến:
\begin{itemize}
    \item Công suất ngưng tụ cần thiết giảm tương ứng
    \item Van làm mát có thể giảm thêm, tiết kiệm thêm nước làm mát
\end{itemize}

Với $Q_{vaporize} = 2641$ W và van làm mát điều chỉnh để cân bằng:
\begin{equation}
    Q_{condenser,optimal} = Q_{vaporize,optimal} = 2641 \text{ W}
\end{equation}

Ước tính độ mở van tương ứng: khoảng 30--35\% (giảm thêm so với 53\% khi reboiler chạy 100\%).
...
\subsubsection{Chế độ tối ưu toàn diện (PWM reboiler + điều khiển van)}

Kết hợp điều khiển PWM công suất reboiler với điều khiển van nước làm mát để tối ưu hóa cả điện năng và nước làm mát.

Kết quả tính toán trạng thái xác lập (ước tính):
\begin{itemize}
    \item Áp suất đỉnh tháp: 1.0 bar (duy trì ổn định)
    \item Công suất reboiler: 3066 W (PWM 51\%)
    \item Van nước làm mát: ~33\%
    \item Lưu lượng nước làm mát: ~2.4 L/min
    \item Công suất ngưng tụ: 2641 W
    \item Nhiệt độ đỉnh: 78.3°C
\end{itemize}
...
Bảng \ref{tab:sim_productivity_comparison} so sánh kết quả tính toán giữa ba phương án vận hành.

\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Thông số} & \textbf{Baseline} & \textbf{Điều khiển van} & \textbf{PWM + Van} & \textbf{Đơn vị} \\
\hline
Công suất reboiler & 6000 & 6000 & 3066 & W \\
\hline
Độ mở van & 100 & 53 & ~33 & \% \\
\hline
Công suất ngưng tụ & 9113 & 5282 & 2641 & W \\
\hline
Lưu lượng nước & 7.2 & 3.8 & ~2.4 & L/min \\
\hline
Tiết kiệm điện & -- & 0\% & 49\% & \\
\hline
Tiết kiệm nước & -- & 47\% & ~67\% & \\
\hline
\end{tabular}
...
\begin{itemize}
    \item \textbf{Điều khiển van (so với baseline):} Giảm 47\% lưu lượng nước làm mát, không tiết kiệm điện.
    \item \textbf{PWM + Van (so với baseline):} Giảm 49\% điện năng reboiler và ~67\% lưu lượng nước làm mát.
\end{itemize}
