...
\begin{equation}
    Q_{reboiler} = Q_{\text{nhập liệu}} + Q_{\text{hóa hơi}}
\end{equation}

\textbf{Công suất reboiler hiệu dụng:}

Công suất điện đầu vào 6000 W không hoàn toàn chuyển thành nhiệt hữu ích do sụt áp trên dây dẫn, tiếp điểm và tổn hao nhiệt ra môi trường. Ước tính tổn hao 10\%, công suất reboiler hiệu dụng:
\begin{equation}
    Q_{reboiler} = 6000 \times 0.90 = 5400 \text{ W}
\end{equation}

\textbf{Nhiệt gia nhiệt nhập liệu ($Q_{\text{nhập liệu}}$):}
...
Nhiệt gia nhiệt:
\begin{equation}
    Q_{\text{nhập liệu}} = \dot{m}_F \cdot C_p \cdot \Delta T
\end{equation}
với:
\begin{itemize}
...
Kết quả: $Q_{\text{nhập liệu}} = 118$ W

\textbf{Nhiệt hóa hơi ($Q_{\text{hóa hơi}}$):}

Từ cân bằng năng lượng tại reboiler:
\begin{equation}
    Q_{\text{hóa hơi}} = Q_{reboiler} - Q_{\text{nhập liệu}} = 5400 - 118 = 5282 \text{ W}
\end{equation}
...
\begin{equation}
    Q_{\text{ngưng tụ, tối ưu}} = Q_{\text{hóa hơi}} = 5282 \text{ W}
\end{equation}

Nếu $Q_{\text{ngưng tụ}} < Q_{\text{hóa hơi}}$: hơi không ngưng tụ hết, áp suất tháp tăng.

Nếu $Q_{\text{ngưng tụ}} > Q_{\text{hóa hơi}}$: dung dịch ngưng tụ bị làm lạnh dưới nhiệt độ sôi (subcooling), lãng phí nước làm mát.

\subsubsection{Tính toán lưu lượng nước làm mát}

Công suất ngưng tụ được tính từ cân bằng năng lượng phía nước làm mát:
\begin{equation}
    Q_{\text{ngưng tụ}} = \dot{m}_{water} \cdot C_p \cdot (T_{out} - T_{in})
\end{equation}
...
Với lưu lượng tối đa $F = 7.2$ L/min = 0.12 kg/s, nếu condenser chỉ cần hấp thụ $Q_{\text{hóa hơi}} = 5282$ W:
\begin{equation}
    T_{out} = T_{in} + \frac{Q_{\text{hóa hơi}}}{\dot{m}_{water} \cdot C_p} = 30 + \frac{5282}{0.12 \times 4186} = 40.5^\circ\text{C}
\end{equation}

Nhiệt độ nước ra thấp (40.5$^\circ$C) so với nhiệt độ ngưng tụ (78$^\circ$C) cho thấy lưu lượng nước đang dư thừa đáng kể. Phần công suất làm mát dư thừa sẽ làm lạnh dung dịch ngưng tụ xuống dưới nhiệt độ sôi (subcooling).

\textbf{Tính toán lưu lượng tối ưu:}

Để vận hành tối ưu với $Q_{\text{ngưng tụ}} = Q_{\text{hóa hơi}} = 5282$ W, lưu lượng nước cần thiết phụ thuộc vào nhiệt độ nước ra:
\begin{equation}
    \dot{m}_{optimal} = \frac{Q_{\text{hóa hơi}}}{C_p \cdot (T_{out} - T_{in})}
\end{equation}

Trong thiết kế thiết bị trao đổi nhiệt, chênh lệch nhiệt độ tiếp cận (approach temperature) thường được chọn khoảng 10$^\circ$C để đảm bảo động lực truyền nhiệt đủ lớn. Với $T_{\text{ngưng tụ}} = 78^\circ$C:
\begin{equation}
    T_{out} = T_{\text{ngưng tụ}} - \Delta T_{approach} = 78 - 10 = 68^\circ\text{C}
\end{equation}
...
\begin{itemize}
    ...
    \item Công suất ngưng tụ: 5282 W (cân bằng với $Q_{\text{hóa hơi}}$)
    \item Nhiệt độ đỉnh: 78.3°C
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Cân bằng năng lượng}: Bộ điều khiển tự động điều chỉnh độ mở van để công suất ngưng tụ (5282 W) cân bằng với nhiệt hóa hơi ($Q_{\text{hóa hơi}} = Q_{reboiler} - Q_{\text{nhập liệu}} = 5400 - 118 = 5282$ W). Điều này đảm bảo quá trình ngưng tụ hiệu quả mà không lãng phí nước làm mát.
...
\begin{itemize}
    ...
    \item Nước chỉ cần hấp thụ $Q_{\text{hóa hơi}} = 5282$ W
...
Để kiểm chứng hiệu quả của chiến lược điều khiển van làm mát, sử dụng độ chênh nhiệt độ giữa đỉnh tháp và dung dịch ngưng tụ:
\begin{equation}
    \Delta T_{subcool} = T_{\text{đỉnh}} - T_{\text{ngưng tụ}}
\end{equation}

Trong đó:
\begin{itemize}
    \item $T_{\text{đỉnh}}$: Nhiệt độ hơi tại đỉnh tháp (đo bằng cảm biến nhiệt độ đỉnh) $\approx$ 78.4°C
    \item $T_{\text{ngưng tụ}}$: Nhiệt độ dung dịch ngưng tụ sau condenser (đo bằng cảm biến nhiệt độ dung dịch ngưng tụ)
\end{itemize}

Ý nghĩa của $\Delta T_{subcool}$:
\begin{itemize}
    \item $\Delta T_{subcool} \approx 0$°C: Dung dịch ngưng tụ ở nhiệt độ sôi, không bị làm lạnh quá mức -- vận hành tối ưu.
    \item $\Delta T_{subcool} > 0$°C: Dung dịch ngưng tụ bị làm lạnh dưới nhiệt độ sôi (subcooling) -- lãng phí nước làm mát.
\end{itemize}
...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Chế độ} & \textbf{Độ mở van} & \textbf{$\Delta T_{subcool}$ dự kiến} \\
\hline
Baseline & 100\% & $>$ 10°C \\
\hline
Tối ưu & 28\% & $\approx$ 0°C \\
\hline
\end{tabular}
