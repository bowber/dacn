...
Factory I/O là phần mềm mô phỏng 3D các hệ thống tự động hóa công nghiệp. Ứng dụng trong đào tạo:
\begin{itemize}
    \item Tạo môi trường thực hành an toàn, không rủi ro hư hỏng thiết bị
    \item Mô phỏng nhiều kịch bản sản xuất: băng tải, phân loại, điều khiển mức, palletizing...
    \item Kết nối với PLC thực hoặc mô phỏng (PLCSIM) để kiểm tra chương trình
    \item Tiết kiệm chi phí đầu tư thiết bị thực cho cơ sở đào tạo
    \item Cho phép sinh viên luyện tập nhiều lần, điều chỉnh thông số dễ dàng
\end{itemize}

\textbf{Câu 2: Mô tả các bước kết nối PLCSIM với Factory I/O?}

\begin{enumerate}
    \item Tạo project trong TIA Portal với PLC S7-1200/1500
    \item Cấu hình địa chỉ IP cho PLC (ví dụ: 192.168.0.1)
    \item Khởi chạy PLCSIM và download chương trình
    \item Mở Factory I/O, vào \texttt{File > Driver Configuration}
    \item Chọn Driver: \texttt{Siemens S7-PLCSIM}
    \item Nhập địa chỉ IP của PLCSIM
    \item Cấu hình vùng nhớ I/O (Input/Output addresses)
    \item Nhấn \texttt{Connect} để kiểm tra kết nối
    \item Map các tag trong Factory I/O với địa chỉ PLC tương ứng
\end{enumerate}
...
\hline
\textbf{Tiêu chí} & \textbf{Mô phỏng} & \textbf{Thiết bị thực} \\
\hline
Chi phí & Thấp (chỉ cần phần mềm) & Cao (mua, bảo trì thiết bị) \\
\hline
An toàn & Hoàn toàn an toàn & Có rủi ro hư hỏng, tai nạn \\
\hline
Độ chính xác & Phụ thuộc mô hình & Phản ánh thực tế \\
\hline
Linh hoạt & Dễ thay đổi, reset nhanh & Mất thời gian thiết lập lại \\
\hline
Nhiễu & Không có nhiễu thực tế & Có nhiễu môi trường \\
\hline
Kỹ năng thực hành & Hạn chế (không thao tác thật) & Rèn luyện kỹ năng thực tế \\
\hline
\end{tabular}
\caption{So sánh mô phỏng và thực hành trên thiết bị thực}
\end{table}
