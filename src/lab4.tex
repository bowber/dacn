...
Bài thí nghiệm này nhằm khảo sát ảnh hưởng của các thông số $K_P$, $K_I$, $K_D$ của bộ điều khiển PID đến chất lượng điều khiển. Thông qua việc thay đổi từng thông số và ghi nhận đáp ứng của hệ thống, sinh viên hiểu rõ hơn về vai trò của từng thành phần trong bộ điều khiển PID.
...
Bài thí nghiệm này sử dụng cùng hệ thống thiết bị như Bài thí nghiệm 3: hệ thống điều khiển mức chất lỏng với máy tính cài đặt phần mềm điều khiển và giao diện giám sát Halalab.

Trên giao diện Halalab, sinh viên chọn bộ điều khiển PID và nhập các thông số $K_P$, $K_I$, $K_D$ để khảo sát ảnh hưởng của chúng đến chất lượng điều khiển:
\begin{itemize}
    \item Đồ thị hiển thị giá trị đại lượng cần điều khiển theo thời gian thực.
    \item Giá trị cài đặt của đại lượng cần điều khiển ($y_{sp}$).
    \item Lựa chọn bộ điều khiển PID.
    \item Ô nhập các thông số bộ điều khiển PID ($K_P$, $K_I$, $K_D$).
    \item Nút Auto để bắt đầu khảo sát với bộ thông số đã nhập.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Chọn bộ điều khiển PID trên giao diện điều khiển.
\end{enumerate}

\subsubsection{Khảo sát ảnh hưởng các thông số bộ điều khiển PID}
...
\subsubsection{Kết quả khảo sát ảnh hưởng thông số PID}
...
\begin{itemize}
    ...
    \item Khi tăng $K_I$: Sai lệch tĩnh được triệt tiêu nhưng có thể gây dao động.
...
\begin{itemize}
    ...
    \item Xác định được quy luật ảnh hưởng của các thông số và cách chỉnh định bộ điều khiển PID cho phù hợp.
\end{itemize}

\textbf{Khuyến nghị:}
\begin{itemize}
    \item Trong thực tế, cần tinh chỉnh các thông số dựa trên yêu cầu cụ thể của quá trình.
    \item Cần cân bằng giữa các chỉ tiêu chất lượng: thời gian đáp ứng, độ quá điều chỉnh và sai lệch tĩnh.
\end{itemize}
...
\begin{tikzpicture}[node distance=1.5cm]
    ...
    \node[block, right=1.3cm of sum, minimum width=4em] (pid) {PID};
    \node[block, right=1.5cm of pid, minimum width=4em] (actuator) {Van/Bơm};
    \node[block, right=1.5cm of actuator, minimum width=4em] (process) {Quá trình};
    ...
    % Sensor
    \node[block, below=1.3cm of actuator, minimum width=4em] (sensor) {Cảm biến};
...
\begin{itemize}
    \item Bộ điều khiển: Phần mềm Halalab với thuật toán PID
    \item Thiết bị đo: Cảm biến mức chất lỏng
    \item Thiết bị chấp hành: Bơm
    \item Giao diện: Phần mềm Halalab hiển thị đồ thị và cho phép cài đặt thông số $K_P$, $K_I$, $K_D$
\end{itemize}
...
\begin{enumerate}[label={[\arabic*]}]
    \item B. N. Pha, \textit{Điều khiển Quá trình Công nghệ Hoá học -- Cơ sở Điều khiển Quá trình}, Quyển 1. Trường Đại học Bách khoa, ĐHQG-HCM, 2021.
    \item B. N. Pha và P. H. H. P. Lợi, \textit{Điều khiển Quá trình Công nghệ Hoá học -- Hướng dẫn Thí nghiệm – Thực hành Cơ sở Điều khiển Quá trình}, Quyển 2. Trường Đại học Bách khoa, ĐHQG-HCM, 2021.
\end{enumerate}
